  ...
  trees: [
    ...
    /* ===== 1. HORMONE CLASSES AND SIGNALING ===== */
    {
      ...
      root: {
        ...
        note: "Almost everything else in endocrine physiology follows from two questions: is this hormone water soluble or lipid soluble, and where is its receptor. Get those two right and the rest is predictable.",
        children: [

          { id: "chem-classes", label: "Chemical classes", type: "structure", edge: "type-of",
            note: "Hormones sort into three chemical families. The family predicts storage, transport, receptor location, speed, and how long the effect lasts.",
            children: [

              { id: "peptide-hormones", label: "Peptide and protein hormones", type: "structure", edge: "type-of",
                note: "The largest group by far. Chains of amino acids, from very short to quite large.",
                children: [
                  { id: "peptide-solubility", label: "Water soluble, made in advance and stored in vesicles", type: "process", edge: "causes",
                    note: "Because they dissolve in plasma, they can be made ahead of time and held ready, which is why peptide responses are fast." },
                  { id: "peptide-receptor", label: "Bind surface receptors and work through second messengers", type: "process", edge: "causes",
                    note: "A water soluble hormone cannot cross the lipid membrane, so it has to hand the message off at the surface." },
                  { id: "peptide-kinetics", label: "Fast onset, short half life measured in minutes", type: "process", edge: "causes" },
                  { id: "peptide-examples", label: "Insulin, glucagon, PTH, ADH, oxytocin, GH, ACTH, TSH, FSH, LH, prolactin, calcitonin, ANP", type: "structure", edge: "type-of",
                    children: [
                      { id: "cl-insulin-injected", label: "Insulin has to be injected, not swallowed", type: "clinical", edge: "seen-as",
                        note: "Digestive enzymes break peptides into amino acids. An oral insulin tablet would be digested like any other protein before it ever reached the blood." }
                    ] }
                ] },

              { id: "amine-hormones", label: "Amine hormones", type: "structure", edge: "type-of",
                note: "Built from a single amino acid. This group is worth watching because its members do not all behave the same way.",
                children: [
                  { id: "amine-tyrosine", label: "Made from tyrosine: thyroid hormones and catecholamines", type: "process", edge: "requires" },
                  { id: "amine-catecholamine-behavior", label: "Catecholamines behave like peptides", type: "process", edge: "causes",
                    note: "Water soluble, stored in vesicles, surface receptors, fast and brief." },
                  { id: "amine-thyroid-behavior", label: "Thyroid hormones behave like steroids", type: "process", edge: "causes",
                    note: "Lipid soluble, carried on plasma proteins, nuclear receptors, slow and long lasting. Same chemical family as catecholamines, completely different behavior." },
                  { id: "amine-melatonin", label: "Melatonin is made from tryptophan", type: "process", edge: "requires" }
                ] },

              { id: "steroid-hormones", label: "Steroid hormones", type: "structure", edge: "type-of",
                note: "All built on the same four-ring cholesterol backbone. Small chemical differences between them produce very different effects.",
                children: [
                  { id: "steroid-source", label: "Made from cholesterol", type: "process", edge: "requires" },
                  { id: "steroid-ondemand", label: "Lipid soluble, so they are made on demand rather than stored", type: "process", edge: "causes",
                    note: "A lipid soluble molecule would diffuse straight out of any storage vesicle, so the cell cannot stockpile it. Steroid output is controlled at the synthesis step instead." },
                  { id: "steroid-receptor", label: "Cross the membrane and bind intracellular receptors", type: "process", edge: "causes",
                    note: "The hormone-receptor complex binds DNA and changes which genes are transcribed. That means new protein has to be made before anything happens." },
                  { id: "steroid-kinetics", label: "Slow onset, long half life measured in hours to days", type: "process", edge: "causes",
                    children: [
                      { id: "cl-steroid-onset", label: "Steroids given for an acute attack do not work immediately", type: "clinical", edge: "seen-as",
                        note: "In an acute asthma attack, a bronchodilator opens the airway within minutes while the steroid works on inflammation over hours. Both are given, and they are not interchangeable." }
                    ] },
          ...
          { id: "hormone-transport", label: "Transport in the blood", type: "process", edge: "contains",
            note: "Getting to the target is a problem only for the lipid soluble hormones, because plasma is water.",
            children: [
              { id: "transport-free", label: "Water soluble hormones travel free in plasma", type: "process", edge: "causes",
                children: [
                  { id: "transport-clearance", label: "Cleared quickly by kidney and liver", type: "process", edge: "causes" }
                ] },
              { id: "transport-bound", label: "Lipid soluble hormones travel bound to carrier proteins", type: "process", edge: "causes",
                children: [
                  { id: "transport-tbg", label: "Thyroxine binding globulin carries thyroid hormone", type: "structure", edge: "type-of" },
                  { id: "transport-cbg", label: "Corticosteroid binding globulin carries cortisol", type: "structure", edge: "type-of" },
                  { id: "transport-shbg", label: "Sex hormone binding globulin carries sex steroids", type: "structure", edge: "type-of" },
                  { id: "transport-albumin", label: "Albumin carries many hormones non-specifically", type: "structure", edge: "type-of" }
                ] },
              { id: "transport-free-fraction", label: "Only the free, unbound fraction is biologically active", type: "control", edge: "causes",
                note: "Bound hormone is a reservoir. It cannot enter a cell, but it protects the hormone from being cleared, which is why lipid soluble hormones last so much longer.",
                children: [
                  { id: "cl-tbg-pregnancy", label: "Pregnancy raises total T4 without causing hyperthyroidism", type: "clinical", edge: "seen-as",
                    note: "Estrogen raises {{thyroxine binding globulin}}, so more hormone is carried and the total level rises. The free fraction, which is what the tissues actually see, stays normal. This is why free T4 is the more useful measurement." }
                ] }
            ] },

          { id: "receptor-signaling", label: "Receptor location and signaling pathways", type: "process", edge: "contains",
            note: "The receptor is where the hormone stops and the cell's own machinery takes over.",
            children: [
              { id: "gpcr-camp", label: "G protein coupled receptors, cAMP pathway", type: "process", edge: "type-of",
                children: [
                  { id: "gs-pathway", label: "Gs activates adenylyl cyclase, raising cAMP and activating protein kinase A", type: "process", edge: "causes" },
                  { id: "gi-pathway", label: "Gi lowers cAMP", type: "process", edge: "causes" },
                  { id: "camp-users", label: "Used by glucagon, ACTH, TSH, PTH, and catecholamines at beta receptors", type: "structure", edge: "acts-on" }
                ] },
              { id: "gpcr-ip3", label: "G protein coupled receptors, IP3 and calcium pathway", type: "process", edge: "type-of",
                children: [
                  { id: "gq-pathway", label: "Gq activates phospholipase C, producing IP3 and DAG and releasing stored calcium", type: "process", edge: "causes" },
                  { id: "ip3-users", label: "Used by ADH at V1 receptors, TRH, GnRH, oxytocin, and angiotensin II", type: "structure", edge: "acts-on" }
                ] },
              { id: "rtk", label: "Receptor tyrosine kinases", type: "process", edge: "type-of",
                children: [
                  { id: "rtk-mechanism", label: "The receptor phosphorylates itself, then signals through IRS and PI3K", type: "process", edge: "causes" },
                  { id: "rtk-users", label: "Used by insulin and IGF-1", type: "structure", edge: "acts-on" }
                ] },
              { id: "jak-stat", label: "JAK-STAT receptors", type: "process", edge: "type-of",
                children: [
                  { id: "jak-mechanism", label: "The receptor recruits JAK, which phosphorylates STAT, which enters the nucleus", type: "process", edge: "causes" },
                  { id: "jak-users", label: "Used by growth hormone, prolactin, leptin, and erythropoietin", type: "structure", edge: "acts-on" }
                ] },
              { id: "nuclear-receptors", label: "Intracellular and nuclear receptors", type: "process", edge: "type-of",
                children: [
                  { id: "nuclear-mechanism", label: "The hormone-receptor complex binds DNA and changes transcription", type: "process", edge: "causes" },
                  { id: "nuclear-users", label: "Used by steroids, thyroid hormone, and calcitriol", type: "structure", edge: "acts-on" }
                ] },
              { id: "cgmp", label: "Membrane guanylyl cyclase, cGMP pathway", type: "process", edge: "type-of",
                children: [
                  { id: "cgmp-users", label: "Used by atrial natriuretic peptide", type: "structure", edge: "acts-on" }
                ] }
            ] },

          { id: "target-responsiveness", label: "Target cell responsiveness", type: "control", edge: "contains",
            note: "The same blood level of a hormone can produce very different effects in two people, because the target cell has a say.",
            children: [
              { id: "receptor-number", label: "Number of receptors on the target cell", type: "control", edge: "causes",
                children: [
                  { id: "upregulation", label: "Up-regulation adds receptors when hormone levels are low", type: "process", edge: "causes" },
                  { id: "downregulation", label: "Down-regulation removes receptors when hormone levels stay high", type: "process", edge: "causes",
                    children: [
                      { id: "cl-insulin-resistance", label: "Chronically high insulin contributes to insulin resistance", type: "clinical", edge: "seen-as",
              ...
              { id: "receptor-affinity", label: "How tightly the receptor binds its hormone", type: "control", edge: "causes" }
            ] },

          { id: "hormone-interactions", label: "How hormones interact with each other", type: "control", edge: "contains",
            children: [
              { id: "permissiveness", label: "Permissiveness: one hormone must be present for another to work at all", type: "control", edge: "type-of",
                children: [
                  { id: "perm-thyroid", label: "Thyroid hormone is permissive for catecholamine effects", type: "process", edge: "causes",
                    note: "Thyroid hormone increases the number of beta adrenergic receptors. Without it, the same amount of epinephrine produces much less effect." }
                ] },
              { id: "synergism", label: "Synergism: two hormones together produce more than the sum of each alone", type: "control", edge: "type-of",
                children: [
                  { id: "syn-glucose", label: "Glucagon, cortisol, and epinephrine together raise glucose more than expected", type: "process", edge: "causes" }
                ] },
              { id: "antagonism", label: "Antagonism: one hormone opposes another", type: "control", edge: "type-of",
                children: [
                  { id: "ant-insulin-glucagon", label: "Insulin and glucagon oppose each other", type: "process", edge: "causes" }
                ] }
            ] }
        ]
      }
